  ...
  <Layout>
    <template v-slot:conteudo>
      <div id="opcoes-conteudo">
        <Link
          v-if="$page.props.inserir_consulta"
        ...
        <form @submit.prevent="filtro.post('/filtro/consulta')">
          <input
            v-model="filtro.date_inicial"
            type="date"
            name="date"
            id="date"
          />
          <input
            v-model="filtro.date_final"
            type="date"
            name="date"
            id="date"
          />
        ...
        <table class="minimal-table">
          ...
          <tbody>
            <tr v-for="consulta in consultas" :key="consulta.id">
              ...
              <td>
                <a
                  v-if="$page.props.apagar_consulta"
                  class="delete"
                  style="background-color: blue"
                  :href="`/emitir/comprov/${consulta.identificacao}`"
                  >Comprovante</a
                >
              </td>
              ...
              <td>
                <Link
                  v-if="$page.props.concluir_consulta"
                  class="status-concluido"
                  :href="'/concluir/consulta/' + consulta.identificacao"
                  >Concluir</Link
                >
              </td>
              <td>
                <a
                  v-if="$page.props.cancelar_consulta"
                  class="status-cancelado"
                  href="#"
                  @click.prevent="abrirFormulario(consulta.identificacao)"
                  >Cancelar</a
                >
              </td>
              <td>
                <Link
                  v-if="$page.props.apagar_consulta"
                  class="delete"
                  :href="'/delete/consulta/' + consulta.identificacao"
                  >X</Link
                >
              </td>
            </tr>
      ...
      <div v-if="showForm" class="modal-sobreposto">
        <div class="modal-content">
          ...
          <form @submit.prevent="formcancel.post('/cancelar/consulta')">
            ...
            <button id="fechar" type="button" @click.prevent="fecharModal">
              Fechar
            </button>
          </form>
        </div>
